...
- `README.md` and `README.en.md` are the Chinese and English human-facing entrypoints. They must explain the twelve Skills whose canonical source is stored in this repository in detail, while external S Paper Skills and PaperTrace Skills receive only a concise overview and canonical links.
- `20-project-build/README.md` and `README.en.md` are bilingual module-level technical architecture maps for the six project-build Skills. They explain ownership, SCI/STAT/ENG/GOV categories, independent state axes, and lifecycle routing, while the root README pair remains the repository-wide overview.
...
- The hub directly maintains six project-build core Skills under `20-project-build/`, four reusable core Skills under `50-core-utils/`, `academic-figure-workflow` under `10-paper-build/`, and `logic-chain-tutor` under `90-personal/`.
...
- `research-management-pipeline` is the repeated post-onboarding lifecycle entry. It locates the unique Governance registry, summarizes SCI/STAT/ENG/GOV coverage, seven independent state axes, gates, gaps, and amendment impact, then routes one smallest next action. It does not execute research, run adapters, create `.agents`, choose methods, or grant authorization.
...
- `research-workspace-governance` owns domain-neutral governance: asset roles, locations, SCI/STAT/ENG/GOV contract instances, seven state axes, provenance, evidence, amendments, invalidation, migration, retention, deletion boundaries, optional project contracts, relocation maps, and derived traceability. It validates metadata and review boundaries without performing domain science.
...
- `20-project-build/research-workspace-governance/scripts/validate_contract_registry.py`
