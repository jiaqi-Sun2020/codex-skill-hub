...
- `project-agent-generator-skill` and `research-project-pipeline` are one-time onboarding components. The Generator creates missing project Agent scaffolding; its `--force` refresh is legacy break-glass recovery, not routine maintenance. The Pipeline coordinates initial discovery, design, review, controlled change, and delegated verification only; its readiness report keeps onboarding, governance, domain validation, execution authorization, and claim support distinct. Before an actual submission it routes to `project-submission-audit`, and after verified work it may route to `handoff` without letting either substitute for governance or scientific validation.
